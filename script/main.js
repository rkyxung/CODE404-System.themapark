...
import {
  errormessage
} from "./errorThemapark.js"; // errorHover.js에서 errormessage 함수 가져오기
...
function Puzzle() {
  ...
  if (currentPuzzle === 2 && !puzzle02once) {
...
function main() {
  ...
  startGame({
    onError: () => {}, // 현재는 onError 콜백 미사용 (추후 필요시 정의 가능)
    glitch, // glitch 효과 함수 전달
    timer, // 타이머 함수 전달
    errormessage, // 에러 메시지 출력 함수 전달
  });
